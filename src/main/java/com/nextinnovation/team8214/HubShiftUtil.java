...
public class HubShiftUtil {
  ...
  private static double shiftTimeOffset = 0.0;
  ...
  private static final double minTimeOfFlight = 0.5;
  private static final double maxTimeOfFlight = 1.0;
  ...
  @Getter @Setter private static boolean forceActive = false;
  ...
  public static void restartShiftTimer() {
    shiftTimer.restart();
  }

  public static void resetShiftTimer(double newElapsedTime) {
    shiftTimeOffset = newElapsedTime - shiftTimer.get();
  }

  public static double getShiftTime() {
    return shiftTimer.get() + shiftTimeOffset;
  }

  private static boolean[] getSchedule() {
    boolean[] currentSchedule;
    var startAlliance = getFirstActiveAlliance();
    currentSchedule =
  ...
  private static ShiftInfo getShiftInfo(
    ...
    var currentTime = getShiftTime();
    var stateTimeElapsed = getShiftTime();
    var stateTimeRemaining = 0.0;
    var active = false;
    var currentShift = ShiftEnum.DISABLED;
    ...
    if (DriverStation.isAutonomousEnabled()) {
      ...
      for (var i = 0; i < shiftStartTimes.length; i++) {
    ...
    return new ShiftInfo(currentShift, stateTimeElapsed, stateTimeRemaining, active || forceActive);
  }
